...
Entering insert state at the end of line (evil-append-line) makes sense in this mode, whereas (evil-append) does not.
...
When you use the mode, you might find yourself only using the (evil-paste-before) command.
...
The new =P= (evil-paste-after) is still bound and can be useful, but you can contextually replace it with =jp= or =lp=.
...
Note that Vim's behavior is still available so e.g. =~= (evil-invert-char) will work on the (char-after) and move forward.
...
In practice =C-v= is still usable and useful but it does have a few bugs tied to the end of line position.
